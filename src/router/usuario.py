...
def registrar(usuario: Usuario) -> JSONResponse:
    ...
    try:
        ...
        if DATA.find_one({"email": usuario_dict["email"]}):
            return JSONResponse(
                status_code=400, content={"msg": "Ya existe un usuario con ese correo."}
            )
        ...
        return JSONResponse(
            status_code=201, content={"msg": "Usuario registrado correctamente"}
        )

    except Exception as e:
        return JSONResponse(
            status_code=500, content={"msg": f"Ocurrio un error inesperado: {e}"}
        )
...
def perfil(token: str = Depends(OA2)) -> JSONResponse:
    ...
    try:
        ...
        if not user:
            return JSONResponse(
                status_code=404, content={"data": "Usuario no encontrado"}
            )
        ...
        return JSONResponse(
            status_code=500,
            content={"data": f"Error al obtener la información del usuario: {e}"},
        )
...
def actualizar(usuario: UsuarioActualizar, token: str = Depends(OA2)) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse(
            content={"msg": "Usuario actualizado correctamente"}, status_code=200
        )
    except Exception as e:
        return JSONResponse(
